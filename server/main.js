...
const globRouter = (item) => {
    const apiJsonPath = item && item.substring(item.indexOf(prefix), item.length);
    ...
    // 记录路由
    const path = `http://127.0.0.1:${port}${apiPath}`;
...
router.post("/upload", (ctx) => {
    try {
        ...
        const writer = fs.createWriteStream(`api/v1/${fileName}`);
        reader.pipe(writer);
    } catch (e) {
        ctx.throw(e);
    }
